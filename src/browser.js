...
export default {
    signCreateContract:function(nonce,prikey,exdata,args){
        let from = getPrikey(nonce,prikey);
        let result = lib.rpc.signCreateContract(from,exdata,JSON.parse(args));
    
        return result.tx;
    },
    signCallContract:function(nonce,prikey,exdata,args){
        let from = getPrikey(nonce,prikey);
        let result = lib.rpc.signCallContract(from,exdata,JSON.parse(args));
    ...
    signTransferToken:function(nonce,prikey,args){
        ...
        let from = getPrikey(nonce,prikey);
        let result = lib.rpc.signCreateToken(from,exdata,JSON.parse(args));
        return result.tx;
    },
